...
@Component
public class AwaitingGenderHandler extends BotStateHandler {

    private final UserStateService userStateService;
    private final MessageSender messageSender;

    @Autowired
    public AwaitingGenderHandler(BotStateService botStateService, UserStateService userStateService, MessageSender messageSender) {
    ...
    public void handle(Message message) {
        ...
        if (message.hasText()) {
            userStateService.updateGender(userId, message.getText());
            setNextState(userId, BotState.AWAITING_PHOTO);
        } else {
            messageSender.sendMessage(userId, "Пожалуйста, введите ваш пол.");
        }
    }
}